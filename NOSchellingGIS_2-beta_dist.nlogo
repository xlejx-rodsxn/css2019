...
turtles-own [ id popdata totalpop maxpop uti utidis  ]
...
to move-one
  ...
  let ethnic-threshold (ifelse-value        ; ethnic threshold depends on ethnic membership (the same for all ses groups of the same ethnic group)
    ethnicity = "ASIAN" [asian_thres]
    ethnicity = "BLACK" [black_thres]
    ethnicity = "WHITEB" [whiteb_thres]
    ethnicity = "OTHER" [other_thres])

  let beta_eth (ifelse-value               ; the relative weight of ethnic utility in the relocation would depend on the ethnicXses of agent.
    ethnicity = "ASIAN" [                  ; different determinism for specific types of agents. Came to be difficult to understand and not interesting.
      (ifelse-value
      ses = "HIGH" [high_asian]
      ses = "MID" [mid_asian]
      ses = "LOW" [low_asian]
...
;"WHITEB" "ASIAN" "BLACK" "OTHER"
  ...
  let options (turtle-set self min-n-of num_alternative other turtles  [distance myself] )
  ask options [
    set uti utility percent-similar-ethnicity ethnicity ethnic-threshold     ; ethnic utility
    set utidis utility-dist (distance myself)                                ; distance utility
  ]

  let target rnd:weighted-one-of options [exp ((beta_eth * uti) + (beta_dis * utidis))] ; here there were a beta determinism for ethnic utility and distance utility. Idea was:  higher beta_dis, the less the agent would leave district, provided equal ethnic utility
...
to color-shape
  foreach gis:feature-list-of bradford [ x ->
    ...
    let val (ifelse-value
      ...
      [[ utility percent-similar-ethnicity "ASIAN" asian_thres] of turt]  ; high concentration, when beta = 0 random and not coupled, makes sense
        (district-color = "utility WHITEB")
      [[ utility percent-similar-ethnicity "WHITEB" whiteb_thres] of turt]
        (district-color = "utility BLACK")
      [[ utility percent-similar-ethnicity "BLACK" black_thres] of turt]
...
to-report percent-similar-ses [ses]
  report sum item (position ses sess) popdata / totalpop
end
...
to-report utility-dist [c]
  report ifelse-value ((1 - c) < 0)[0][1 - c]  ; distance utility, the closer to the caller the lower the distance and higher the utility with limit to 1 (0 distance,  utility 1 is the caller itself).
end                                            ; For negative values, utility is set 0, to debug with rnd (it requires positive weights)
...
district-color
district-color
"ethnfrac ASIAN" "ethnfrac BLACK" "ethnfrac WHITEB" "ethnfrac OTHER" "sesfrac LOW" "sesfrac MID" "sesfrac HIGH" "POP" "ETHNIC-CONCENTRATION" "SES-CONCENTRATION" "AVGERAGE SES" "utility ASIAN" "utility BLACK" "utility WHITEB"
0
...
Will be deleted. I suggest SchellingGIS_2-Rocco.nlogo

Here just to explain:
- movement to closer neighborhood, including distance utility, deleted in the new version
- beta (determinism) for each ethnXses class, ethnic threshold for utility function depending on ethnic group. Too complicated and useless, simplified in new version
