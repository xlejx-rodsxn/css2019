...
to move-one
  ...
  let target rnd:weighted-one-of options [exp ((beta * uti) + (beta_dis * utidis))]    ; now the target is picked up with random utility model p=exp(beta*U)/sum(beta*U), using roulette-wheel and rnd
