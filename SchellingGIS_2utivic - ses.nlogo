...
turtles-own [ id popdata totalpop maxpop utieth utidis utises ]
...
to move-one
  ...
  let ethnic-threshold (ifelse-value
    ethnicity = "ASIAN" [
      (ifelse-value
...
let ses-threshold (ifelse-value
    ethnicity = "ASIAN" [
      (ifelse-value
  ...
  let options (turtle-set self min-n-of num_alternative other turtles  [distance myself] )         ; choice: basket of alternatives as local turtle-set of caller > the own district and a random  district (here spatial limit could be included)

  ask options [
    set utieth utility percent-similar-ethnicity ethnicity ethnic-threshold
       set utises utility percent-similar-ses ses ses-threshold
  ;  set utidis utility-dist (distance myself)
  ] ; options update utility based on % similar of ethnicity rnd picked (1 roll asian,1black..) and threshold (utility reporter)

  let target rnd:weighted-one-of options [exp (  beta_determinism * (utieth + utises ))]    ; now the target is picked up with random utility model p=exp(beta*U)/sum(beta*U), using roulette-wheel and rnd
 ...
 ;   let target rnd:weighted-one-of options [exp (beta * uti)]
...
;report ifelse-value (c = 0) [1] [(1 / c)]
  report ifelse-value ((1 - c) < 0)[0][1 - c]
end
