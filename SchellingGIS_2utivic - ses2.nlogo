...
;"WHITEB" "ASIAN" "BLACK" "OTHER"
  ...
  ask options [set uti utility percent-similar-ethnicity ethnicity ethnic-threshold
    set utidis utility-dist (distance myself)  ] ; options update utility based on % similar of ethnicity rnd picked (1 roll asian,1black..) and threshold (utility reporter)

  let target rnd:weighted-one-of options [exp ((beta_eth * uti) + (beta_dis * utidis))]    ; now the target is picked up with random utility model p=exp(beta*U)/sum(beta*U), using roulette-wheel and rnd
