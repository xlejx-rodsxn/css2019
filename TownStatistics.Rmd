...
```{r}
d <- read_dta("TownData/ethnic_lsoa_town.2011.dta") %>% 
  mutate(fwhite = whiteb/AllUsualResidents,
         fasian2 = asian2/AllUsualResidents,
         fblack = black/AllUsualResidents) %>% 
  group_by(town) %>% 
  summarize_at(vars(AllUsualResidents, whiteb, asian2, black), sum) %>% 
  mutate(Frac_White = whiteb/AllUsualResidents, 
         Frac_Asian = asian2/AllUsualResidents, 
         Frac_Black = black/AllUsualResidents) %>% 
  select(town, starts_with("Frac"))
# d %>% knitr::kable(format.args = list(digits=3))

d %>% ggplot(aes(Frac_White,Frac_Asian, label = as.character(town))) + 
